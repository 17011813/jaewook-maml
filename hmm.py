...
def train(model, saver, sess, exp_string, resume_itr=0):
    ...
    if FLAGS.log:
        train_writer = tf.summary.FileWriter(FLAGS.logdir + '/'+data_name+ '/' + exp_string, sess.graph)
    ...
    for itr in range(resume_itr, FLAGS.pretrain_iterations + FLAGS.metatrain_iterations):
        ...
        if (itr!=0) and itr % SAVE_INTERVAL == 0:
            saver.save(sess, FLAGS.logdir + '/'+data_name+ '/' + exp_string + '/model' + str(itr))
...
def test(model, saver, sess, exp_string, test_num_updates=None):
    ...
    out_filename = FLAGS.logdir + '/'+data_name+ '/' + exp_string + '/' + 'test_ubs' + str(FLAGS.update_batch_size) + '_stepsize' + str(FLAGS.update_lr) + '.csv'
    out_pkl = FLAGS.logdir + '/'+data_name+ '/' + exp_string + '/' + 'test_ubs' + str(FLAGS.update_batch_size) + '_stepsize' + str(FLAGS.update_lr) + '.pkl'
...
def main():
    ...
    if FLAGS.resume or not FLAGS.train: # resume 꺼놓으면 무조건 처음부터 다시 train
        # print("test???")
        model_file = tf.train.latest_checkpoint(FLAGS.logdir + '/'+data_name+ '/' + exp_string)
        if FLAGS.Do_meta:
