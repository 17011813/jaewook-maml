...
def preprocess_data_forecasting(data, time_len, rate, validation_rate, seq_len, pre_len,meta_batch_size):
    ...
    train_x = np.reshape(train_x, (-1, 1, seq_len))
    train_y = np.reshape(train_y, (-1, 1, 1))
    test_x = np.reshape(test_x, (-1, 1, seq_len))
...
def generate_ILI_batch(train_x, train_y, test_x, test_y, meta_batch_size):
    ...
    temp_train_x = train_x[temp_ind]
    temp_train_y = train_y[temp_ind]
    temp_test_x = test_x[temp_ind1]
    temp_test_y = test_y[temp_ind1]
    i = 0
    j = 0
    data_batch = []
    label_batch = []
    ...
    while i < len_data:
        if i + meta_batch_size > len_data:
            break
        mini_batch = temp_train_x[i:i + meta_batch_size]
        mini_label = temp_train_y[i:i + meta_batch_size]
    ...
    while j < len_data1:
        if j + meta_batch_size > len_data1:
            break
        mini_batch1 = temp_test_x[j:j + meta_batch_size]
        mini_label1 = temp_test_y[j:j + meta_batch_size]
