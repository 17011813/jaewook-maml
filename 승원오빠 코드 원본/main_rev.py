...
## Training options
flags.DEFINE_integer('pretrain_iterations', 500, 'number of pre-training iterations.')
flags.DEFINE_integer('metatrain_iterations', 500, 'number of metatraining iterations.') # 15k for omniglot, 50k for sinusoid
...
meta_train_data = np.mat(meta_train_data, dtype=np.float32)
meta_test_data = np.mat(meta_test_data, dtype=np.float32)

scaler = MinMaxScaler()
scaler.fit(meta_train_data[0:int(time_len * FLAGS.train_rate)])
meta_train_data = scaler.transform(meta_train_data)

scaler1 = MinMaxScaler()
scaler1.fit(meta_test_data[0:int(time_len * FLAGS.train_rate)])
meta_test_data = scaler1.transform(meta_test_data)
...
def train(model, saver, sess, exp_string, resume_itr=0):
    ...
    train_train_x, train_train_y, train_test_x, train_test_y = preprocess_data_forecasting(meta_train_data, time_len, train_rate, validation_rate, seq_len, pre_len,meta_batch_size)
    test_train_x, test_train_y, test_test_x, test_test_y = preprocess_data_forecasting(meta_test_data, time_len, train_rate, validation_rate, seq_len, pre_len,meta_batch_size)

    meta_train_train_x, meta_train_train_y, meta_train_test_x, meta_train_test_y = generate_ILI_batch(train_x=train_train_x, train_y=train_train_y, test_x=train_test_x, test_y=train_test_y,meta_batch_size=meta_batch_size)
    meta_test_train_x, meta_test_train_y, meta_test_test_x, meta_test_test_y = generate_ILI_batch(train_x=test_train_x, train_y=test_train_y, test_x=test_test_x, test_y=test_test_y,meta_batch_size=meta_batch_size)
    meta_test_train_x_val, meta_test_train_y_val, meta_test_test_x_val, meta_test_test_y_val = generate_test_ILI_batch(train_x=test_train_x,
                                                                                                   train_y=test_train_y,
                                                                                                   test_x=test_test_x,
                                                                                                   test_y=test_test_y)

    print(meta_train_train_x.shape, meta_train_train_y.shape, meta_train_test_x.shape, meta_train_test_y.shape, 'Meta train data shapes')
    print(meta_test_train_x.shape, meta_test_train_y.shape, meta_test_test_x.shape, meta_test_test_y.shape, 'Meta test data shapes')  # (18, 25, 10, 20) (18, 25, 1, 10)
    ...
    for itr in range(resume_itr, FLAGS.pretrain_iterations + FLAGS.metatrain_iterations):
        ...
        if itr != 0 and itr % TEST_PRINT_INTERVAL == 0:
            predb_list = []
            for j in range(int(meta_test_train_x_val.shape[0])):
                inputa = meta_test_train_x_val[j, :, :, :]
                # print(inputa.shape)
                labela = meta_test_train_y_val[j, :,:, :]
                inputb = meta_test_test_x_val[j, :, :, :]  # b used for testing
                labelb = meta_test_test_y_val[j, :, :, :]
...
def test(model, sess, exp_string, test_num_updates=None):
    np.random.seed(1)
    random.seed(1)
    ...
    for j in range(iter):
        ...
        for i in range(num_testbatch):
            inputa = meta_test_train_x[i, :, :, :]
            labela = meta_test_train_y[i, :, :, :]
...
def get_grad(model, sess, exp_string, test_num_updates=None):
    np.random.seed(1)
    random.seed(1)
    ...
    for i in range(target_num_nodes):
        ...
        for j in range(num_testbatch):
            ...
            inputb = meta_test_test_x[0, :, :, :]  # b used for testing
            labelb = meta_test_test_y[0, :, :, :]
...
def main():
    ...
    get_grad(model, sess, exp_string, test_num_updates)
    #
    # if FLAGS.train:
    #     train(model, saver, sess, exp_string, resume_itr)
    # else:
    #     test(model, sess, exp_string, test_num_updates)
